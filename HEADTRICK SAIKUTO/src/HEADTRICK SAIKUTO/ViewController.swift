...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        HEADTRICKSAIKUTO.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        HEADTRICKSAIKUTO.webView.setNeedsLayout()
    }
    
    func initWebView() {
        HEADTRICKSAIKUTO.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(HEADTRICKSAIKUTO.webView);
        
        HEADTRICKSAIKUTO.webView.uiDelegate = self;
        
        HEADTRICKSAIKUTO.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if pullToRefresh {
            #if !targetEnvironment(macCatalyst)
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: .valueChanged)
            HEADTRICKSAIKUTO.webView.scrollView.addSubview(refreshControl)
            HEADTRICKSAIKUTO.webView.scrollView.bounces = true
            #endif
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = HEADTRICKSAIKUTO.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = HEADTRICKSAIKUTO.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        HEADTRICKSAIKUTO.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !HEADTRICKSAIKUTO.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        HEADTRICKSAIKUTO.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            HEADTRICKSAIKUTO.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                HEADTRICKSAIKUTO.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(HEADTRICKSAIKUTO.webView.estimatedProgress);
    ...
    deinit {
        HEADTRICKSAIKUTO.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: HEADTRICKSAIKUTO.webView)
        }
